...
describe "User pages" do
  ...
  describe "profile page" do
    ...
    describe "friend/unfriend buttons" do
      ...
      describe "added a user" do
        before { visit user_path(other_user) }

        it "should increment the friends user count" do
          expect do
            click_button "Add Friend"
          end.to change(user.friends_users, :count).by(1)
        end

        it "should increment the other user's friendss count" do
          expect do
            click_button "Add Friend"
          end.to change(other_user.friendss, :count).by(1)
        end
      ...
      describe "unfriend a user" do
        ...
        it "should decrement the other user's friendss count" do
          expect do
            click_button "Unfriend"
          end.to change(other_user.friendss, :count).by(-1)
        end
  ...
  describe "edit" do
    ...
    describe "added/friendss" do
    let(:user) { FactoryGirl.create(:user) }
    let(:other_user) { FactoryGirl.create(:user) }
    before { user.addfriend!(other_user) }

    describe "friends users" do
      before do
        sign_in user
        visit added_user_path(user)
      end

      it { should have_title(full_title('Added')) }
      it { should have_selector('h3', text: 'Added') }
      it { should have_link(other_user.name, href: user_path(other_user)) }
    end

    describe "friendss" do
      before do
        sign_in other_user
        visit friendss_user_path(other_user)
      end

      it { should have_title(full_title('Friendss')) }
      it { should have_selector('h3', text: 'Friendss') }
